  ...
  <v-row>
    <v-col cols="12">
      <v-tabs v-model="tab" fixed-tabs dark>
        <v-tab> CƠ SỞ Y TẾ </v-tab>
        <v-tab> DỊCH VỤ - THUỐC - HÓA CHẤT </v-tab>
      </v-tabs>

      <v-tabs-items v-model="tab">
        <v-tab-item> <facility-component-vue /> </v-tab-item>
        <v-tab-item> <service-component-vue /> </v-tab-item>
      </v-tabs-items>
    </v-col>
    <!-- amination -->
...
<script>
import FacilityComponentVue from '../components/FacilityComponent.vue'
import ServiceComponentVue from '../components/ServiceComponent.vue'

export default {
  name: 'MedicalPage',
  components: {
    FacilityComponentVue,
    ServiceComponentVue,
  },
  data() {
    return {
